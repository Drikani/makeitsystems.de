...
- Styling is Bootstrap 5 (via the `bootstrap` gem, see below) plus a small custom layer in [_sass/](_sass) (`_custom.scss`/`_custom-dark.scss`/`_main-variables.scss`/`cookiepopup.scss`/`_roboto.scss`/`_color-variables.scss`), compiled from [css/main.scss](css/main.scss) into `css/main.css`; `_config.yml` sets `sass_dir: _sass`, `style: compact`, and `quiet_deps: true`.
  - **Masonry-style card grids use the `.masonry` utility class (in `_custom.scss`), not a JS library.** Plain CSS multi-column layout (`column-count` + `break-inside: avoid`), used on `index.html`'s hobby cards and `impressum.html`'s Datenschutz cards. This replaced Salvattore (`js/salvattore.js`, `_sass/_salvattore.scss`, the `#grid[data-columns]`/`.item` markup) — removed entirely, no longer a dependency. Native CSS masonry (`grid-template-rows: masonry` / `display: grid-lanes`) was considered but as of mid-2026 only ships unflagged in Safari; Chrome and Firefox still have it behind a flag, so it's not production-safe yet. Revisit once that lands broadly.
