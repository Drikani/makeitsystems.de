...
- Static assets (images) live in [assets/](assets); vendored/plain JS (jQuery 4.0.0, Materialize 1.0.0, Salvattore, cookie popup, color-scheme detection) lives in [js/](js) and is included directly via `<script>` tags, not bundled — there's no package.json. Materialize 1.0.0 (2018) is the actual latest release; the project is archived, so there's nothing newer to pull. Updating these libs means manually fetching new files and diffing the compiled CSS, not a version bump — see the `_main-variables.scss` comments for values upstream Materialize dropped that this site restores (self-hosted Roboto, `.flow-text`/`.footer-copyright` font-weight).
